...
std::vector<PossibleChar> findPossibleCharsInPlate(cv::Mat &imgGrayscale, cv::Mat &imgThresh) {
    ...
    cv::findContours(imgThreshCopy, contours, CV_RETR_LIST, 2);        // find all contours in plate
...
std::string recognizeCharsInPlate(cv::Mat &imgThresh, std::vector<PossibleChar> &vectorOfMatchingChars) {
    ...
    cv::cvtColor(imgThresh, imgThreshColor, cv::ColorConversionCodes::COLOR_GRAY2BGR);       // make color version of threshold image so we can draw contours in color on it
